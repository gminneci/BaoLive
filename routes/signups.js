...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const existing = await dbGet('SELECT id, children as existing_children FROM activity_signups WHERE activity_id = ? AND family_id = ?', [activity_id, family.id]);
    ...
    if (existing) {
        if (!children || children.length === 0) {
            ...
            // Update - check capacity before allowing more children
            const activity = await dbGet('SELECT max_participants FROM activities WHERE id = ?', [activity_id]);
            if (activity && activity.max_participants > 0) {
                const currentTotal = await dbGet(`
                    SELECT SUM(json_array_length(children)) as total_participants
                    FROM activity_signups
                    WHERE activity_id = ?
                `, [activity_id]);
                
                const existingChildCount = JSON.parse(existing.existing_children).length;
                const newChildCount = children.length;
                const otherSignupsTotal = (currentTotal.total_participants || 0) - existingChildCount;
                const wouldBeTotal = otherSignupsTotal + newChildCount;
                
                if (wouldBeTotal > activity.max_participants) {
                    return res.status(400).json({ 
                        error: 'Activity is full',
                        message: `This activity is full (${activity.max_participants} max). Currently ${otherSignupsTotal} children signed up by other families.`
                    });
                }
            }
        ...
        // Create - CHECK CAPACITY FIRST (race condition fix)
        const activity = await dbGet('SELECT max_participants FROM activities WHERE id = ?', [activity_id]);
        if (activity && activity.max_participants > 0) {
            const currentTotal = await dbGet(`
                SELECT SUM(json_array_length(children)) as total_participants
            ...
            const currentParticipants = currentTotal.total_participants || 0;
            const wouldBeTotal = currentParticipants + children.length;
            
            if (wouldBeTotal > activity.max_participants) {
                return res.status(400).json({ 
                    error: 'Activity is full',
                    message: `This activity is full (${activity.max_participants} max). Currently ${currentParticipants} children signed up.`
                });
            }
        }
